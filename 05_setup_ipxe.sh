...
### for V4.6.x or higher

if [[ ${VERSION} =~ ^4\.+([6-9])\.+([0-9]) ]]
then
    curl -Lo /usr/share/nginx/html/ipxe/rhcos-${VERSION}-x86_64-installer-kernel https://mirror.openshift.com/pub/openshift-v4/dependencies/rhcos/${VERSION_DIR}/rhcos-${VERSION}-x86_64-live-kernel-x86_64 ${USE_CACHE}
    curl -Lo /usr/share/nginx/html/ipxe/rhcos-${VERSION}-x86_64-installer-initramfs.img https://mirror.openshift.com/pub/openshift-v4/dependencies/rhcos/${VERSION_DIR}/rhcos-${VERSION}-x86_64-live-initramfs.x86_64.img ${USE_CACHE}
    curl -Lo /usr/share/nginx/html/ocp/rhcos/images/latest/bios.raw.gz https://mirror.openshift.com/pub/openshift-v4/dependencies/rhcos/${VERSION_DIR}/rhcos-${VERSION}-x86_64-live-rootfs.x86_64.img ${USE_CACHE}
fi
